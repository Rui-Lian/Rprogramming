...
set.seed(13234)
library(caret)
trainSA.small <- trainSA[, c(10, 9, 8, 7, 2, 6, 3)]
names(trainSA.small)
trainSA.small$chd <- as.factor(trainSA.small$chd)
...
```{r}
testSA.samll <- testSA[, c(10, 9, 8, 7, 2, 6, 3)]
testSA.samll$chd <- as.factor(testSA.samll$chd)

pred <- predict(ModelFit, newdata = testSA.samll[,-1])
confusionMatrix(pred, testSA.samll$chd)

```

```{r}
pred <- predict(ModelFit, newdata = trainSA.small[,-1])
confusionMatrix(pred, trainSA.small$chd)
```
...
set.seed(33833)
modelFit <- randomForest(y ~. , data = vowel.train, method = "rf", proximity = TRUE)
?varImp
vImp <- varImp(modelFit)
vImp
order(vImp)
```
